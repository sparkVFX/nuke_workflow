...
def _update_node_thumbnail(node, image_path=None):
    ...
    if "postage_stamp" in node.knobs():
        try:
            ...
            print("{}: postage_stamp enabled".format(_tag))
        except Exception as e:
...
def restore_nb_thumbnails():
    ...
    for node in nuke.allNodes("Group"):
        if "is_nb_player" in node.knobs() and node["is_nb_player"].value():
            ...
            restored += 1
    if restored:
        print("[NB] restore: enabled postage_stamp on {} node(s)".format(restored))
...
def update_prompt_read_node(prompt_node, new_image_path):
    ...
    if player_node:
        ...
        # Update the node thumbnail to reflect the new image
